...
soundstream= nn.DataParallel(soundstream)
soundstream.to(device)
state_dict = torch.load('./Checkpoints/200.pt')
soundstream.load_state_dict(state_dict)

# wave_disc= nn.DataParallel(wave_disc)
wave_disc.to(device)

# stft_disc= nn.DataParallel(stft_disc)
stft_disc.to(device)
...
train_dataset = NSynthDataset(audio_dir="./train/train/audio")
...
for epoch in range(1, N_EPOCHS+1):
    ...
    PATH = f'./Checkpoints/{epoch}.pt'
    torch.save(soundstream.state_dict(), PATH)